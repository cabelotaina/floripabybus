...
files.each do |file|
  ...
  info = html.search("//td[2]/h1/text()").inner_text.squish.split(' - ')
  p file
  p info[0]
  ...
  if time_back.length == 1
   ...
   p time_back
   busdayofwee = html.search("//table[3]/tr[1]/td[1]/h1/text()").inner_text
   Busdayofweek.new()
  Busline.new(:number => info[0].to_s, :line => info[1].to_s,:company => info[2].to_s, :card_price => card_price , :money_price => money_price, :time_to_go => trip_time, :time_to_back => time_back).save
end
